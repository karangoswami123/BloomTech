...
export default function MarketPrices() {
  ...
  useEffect(() => {
    ...
    if (cachedData) {
      ...
      loadPrices(); // No cache → fetch default data
    }
  }, []);
  ...
  const loadPrices = async (state?: string, commodity?: string) => {
    ...
    try {
      ...
      if (Array.isArray(data.records)) {
        setPrices(data.records);
        // ✅ Cache only default (no filters) data to avoid overwriting with filtered data
        if (!state && !commodity) {
          localStorage.setItem('cachedPrices', JSON.stringify(data.records));
        }
      } else {
        setPrices([]);
      }
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6">
        <div>
          <h1 className="text-3xl font-bold">{t('marketPrices') || 'Market Prices'}</h1>
          <p className="text-muted-foreground">Real-time commodity prices from markets across India</p>
        </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <p className="font-semibold">{price.commodity}</p>
                            <p className="text-sm text-muted-foreground">{price.market}, {price.district}</p>
                          </div>
                          {trend === 'up' && <TrendingUp className="h-5 w-5 text-green-600" />}
                          {trend === 'down' && <TrendingDown className="h-5 w-5 text-red-600" />}
                          {trend === 'stable' && <Minus className="h-5 w-5 text-muted-foreground" />}
          ...
          <Card>
            ...
            <CardContent>
              {selectedPrice ? (
                ...
                <div className="text-center py-12 text-muted-foreground">Select a commodity to view price trends</div>
              )}
            </CardContent>
          </Card>
        </div>
