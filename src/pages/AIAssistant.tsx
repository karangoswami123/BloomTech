...
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Activity, Send, Mic, Square, Volume2 } from 'lucide-react';
...
export default function AIAssistant() {
  ...
  // 🎤 Start voice recording
    const startRecording = () => {
      ...
      // Use a loose any type for the event and guard access to results for safety
  ...
  // 🗣️ Text-to-speech for assistant responses
...
`;
      ...
      setMessages((prev) => [...prev, assistantMessage]);
      speakText(response.trim()); // 🔊 Speak the AI's response
    } catch (err) {
      console.error(err);
      toast.error('Gemini AI request failed.');
    } finally {
      setLoading(false);
    }
  };
